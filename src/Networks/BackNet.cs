...
namespace LetsGetNeural
{
    ...
    public class BackNet
    {
        ...
        public void Train(double[] expected, double adjustMod)
        {
            for (int i = 0; i < layers.Length; i++)
            {
                if (i == 0)
                ...
                layers[(layers.Length - 1) - i].UpdateWeights(adjustMod);
            }
        }
    ...
    public class Layer
    {
        ...
        public void UpdateWeights(double mod)
        {
            for (int x = 0; x < input.Length; x++)
            {
                for (int y = 0; y < output.Length; y++)
                {
                    weights[x, y] -= weightsDelta[x, y] * mod;// 0.033;
                }
            }
        }
